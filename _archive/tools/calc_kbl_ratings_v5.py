...
OUT_DIR = ROOT / "outputs" / "kbl_ratings_v5"
...
ATTRS = [
    ...
    "볼과함께뛰는속도", "스피드", "민첩성", "힘", "점프력", "체력",
]

OVR_WEIGHTS = {
    "PG": {"패스": 1.9, "볼핸들링": 1.8, "3점슛": 1.25, "자유투": 0.65, "스틸": 1.15, "외곽수비": 1.25, "스피드": 1.15, "민첩성": 1.1, "체력": 0.8, "드라이빙레이업": 0.95, "중거리슛": 0.65, "근거리슛": 0.45, "수비리바운드": 0.25, "인사이드수비": 0.2},
    "SG": {"3점슛": 1.75, "중거리슛": 1.1, "드라이빙레이업": 1.0, "볼핸들링": 1.1, "패스": 0.95, "스틸": 1.05, "외곽수비": 1.2, "스피드": 1.0, "민첩성": 1.0, "체력": 0.75, "자유투": 0.65, "근거리슛": 0.55, "수비리바운드": 0.35},
    "SF": {"3점슛": 1.25, "중거리슛": 0.9, "드라이빙레이업": 0.9, "근거리슛": 0.8, "수비리바운드": 0.8, "스틸": 0.9, "외곽수비": 1.1, "인사이드수비": 0.75, "힘": 0.75, "스피드": 0.8, "점프력": 0.7, "체력": 0.7, "패스": 0.55},
    "PF": {"근거리슛": 1.2, "포스트컨트롤": 1.15, "공격리바운드": 1.15, "수비리바운드": 1.25, "인사이드수비": 1.2, "블록": 0.95, "힘": 1.1, "점프력": 0.75, "체력": 0.65, "중거리슛": 0.65, "3점슛": 0.55, "외곽수비": 0.55, "드라이빙덩크": 0.75},
    "C": {"근거리슛": 1.45, "포스트컨트롤": 1.35, "공격리바운드": 1.25, "수비리바운드": 1.45, "인사이드수비": 1.55, "블록": 1.25, "힘": 1.2, "점프력": 0.75, "체력": 0.6, "스탠딩덩크": 0.9, "드라이빙덩크": 0.65, "패스": 0.35, "3점슛": 0.25, "볼핸들링": 0.2, "외곽수비": 0.35},
}
...
def physical(pos, height, weight, wingspan, dk=0, blk=0, oreb=0):
...
def stamina(minutes, games, fouls, pos):
    position_bonus = {"PG": 2, "SG": 1, "SF": 0, "PF": -1, "C": -2}[pos]
    value = (
        34
        + minutes * 1.55
        + min(games, 54) * 0.22
...
def calc_attrs(row):
    ...
    phys = physical(pos, height, weight, wingspan, dk=dk, blk=blk, oreb=oreb)
    phys["체력"] = stamina(minutes, num(row.get("GP")), num(row.get("PF")), pos)
...
def main():
    ...
    result = {
        "source": SOURCE.name,
        "method": "position_weighted_v5_jump_stamina",
        "notes": [
            "V4 피지컬 산출식을 기반으로 점프력과 체력을 재보정함.",
            "스피드/민첩성은 포지션 기본값에서 키와 체중이 늘수록 감점함.",
            "힘은 체중, 키, 윙스팬이 늘수록 상승함.",
            "점프력은 윙스팬, 키, DK, BLK, OREB가 늘수록 상승하고 체중이 늘수록 감점함.",
            "체력은 출전시간을 최우선으로 보고 GP, PF, 포지션 보정을 함께 반영함.",
    ...
    out = OUT_DIR / "kbl_4teams_ratings_v5.json"
