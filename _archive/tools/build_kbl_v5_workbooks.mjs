...
const outputDir = path.join(root, "outputs", "kbl_ratings_v5");
const data = JSON.parse(await fs.readFile(path.join(outputDir, "kbl_4teams_ratings_v5.json"), "utf8"));
...
summary.getRange("A1").values = [["KBL 4팀 능력치 초안 V5"]];
summary.getRange("A1").format = {
  fill: "#061B4F",
  font: { bold: true, color: "#FFFFFF", size: 16 },
};
summary.getRange("A2:F2").merge();
summary.getRange("A2").values = [["점프력 재보정 / 체력 추가 / OVR 60 유지"]];
...
for (const [team, rows] of Object.entries(data.rows_by_team)) {
  ...
  sheet.getRange("A1").values = [[`${team} 능력치 초안 V5`]];
...
await fs.writeFile(path.join(outputDir, "KBL_4팀_능력치_초안_V5_preview.png"), new Uint8Array(await ratingsPreview.arrayBuffer()));
const ratingsOutput = await SpreadsheetFile.exportXlsx(ratings);
const ratingsPath = path.join(outputDir, "KBL_4팀_능력치_초안_V5.xlsx");
...
guide.getRange("A1").values = [["KBL 능력치 공식 계산기 V5"]];
...
writeTable(rules, 0, 0, ["능력치", "V5 기준"], [
...
await fs.writeFile(path.join(outputDir, "KBL_능력치_공식계산기_V5_preview.png"), new Uint8Array(await formulaPreview.arrayBuffer()));
const formulaOutput = await SpreadsheetFile.exportXlsx(formula);
const formulaPath = path.join(outputDir, "KBL_능력치_공식계산기_V5.xlsx");
