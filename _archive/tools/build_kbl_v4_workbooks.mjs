...
const outputDir = path.join(root, "outputs", "kbl_ratings_v4");
const data = JSON.parse(await fs.readFile(path.join(outputDir, "kbl_4teams_ratings_v4.json"), "utf8"));
...
summary.getRange("A1").values = [["KBL 4팀 능력치 초안 V4"]];
summary.getRange("A1").format = {
  fill: "#061B4F",
  font: { bold: true, color: "#FFFFFF", size: 16 },
};
summary.getRange("A2:F2").merge();
summary.getRange("A2").values = [["피지컬 재보정 / 개별 60 강제 제거 / OVR 60 유지"]];
...
for (const [team, rows] of Object.entries(data.rows_by_team)) {
  ...
  sheet.getRange("A1").values = [[`${team} 능력치 초안 V4`]];
...
await fs.writeFile(path.join(outputDir, "KBL_4팀_능력치_초안_V4_preview.png"), new Uint8Array(await ratingsPreview.arrayBuffer()));
const ratingsOutput = await SpreadsheetFile.exportXlsx(ratings);
const ratingsPath = path.join(outputDir, "KBL_4팀_능력치_초안_V4.xlsx");
...
guide.getRange("A1").values = [["KBL 능력치 공식 계산기 V4"]];
...
writeTable(guide, 2, 0, ["핵심", "내용"], [
  ["정규화", "Min-Max 사용 안 함"],
  ["하한", "개별 능력치 60 강제 하한 제거, OVR만 60 유지"],
  ["상한", "90 이상 구간 압축으로 99 빈도 축소"],
  ["피지컬", "스피드/민첩성/힘/점프력을 포지션별 기준 체형으로 재산출"],
  ["중거리슛", "직접 기록이 없어 추정 중거리득점, FG%, FT%, PTS, 포지션 보정으로 산출"],
]);
writeTable(rules, 0, 0, ["능력치", "V4 기준"], [
...
await fs.writeFile(path.join(outputDir, "KBL_능력치_공식계산기_V4_preview.png"), new Uint8Array(await formulaPreview.arrayBuffer()));
const formulaOutput = await SpreadsheetFile.exportXlsx(formula);
const formulaPath = path.join(outputDir, "KBL_능력치_공식계산기_V4.xlsx");
